/*
	*
	* PBCustomActions.cpp - CustomActions class for PiBreeze
	*
*/

#include "PBCustomActions.hpp"

PBCustomActions::PBCustomActions( QWidget *parent ) : PBDialog( parent ) {

	createGUI();
	setWidgetProperties();
};

void PBCustomActions::createGUI() {
	...
	addBtn = new PBButton( QIcon( ":/icons/list-add.png" ), "Add Action", this );
	addBtn->setShortcut( Settings->Shortcuts.AddCustomAction.at( 0 ) );
	connect( addBtn, SIGNAL( clicked() ), this, SLOT( addCustomAction() ) );

	delBtn = new PBButton( QIcon( ":/icons/list-remove.png" ), "Remove Action", this );
	connect( delBtn, SIGNAL( clicked() ), this, SLOT( removeCustomActions() ) );

	quitBtn = new PBButton( QIcon::fromTheme( "dialog-close" ), "&Close", this );
...
void PBCustomActions::setupTable() {
...
void PBCustomActions::setWidgetProperties() {
...
void PBCustomActions::addCustomAction() {

	PBNewAction *newAction = new PBNewAction( this );
	newAction->exec();
...
void PBCustomActions::editCustomAction( int row, int col ) {
...
void PBCustomActions::removeCustomActions() {
...
void PBCustomActions::updateButtons() {
...
QString PBCustomActions::currentAction() {

	return actionsTable->item( actionsTable->currentRow(), 1 )->text();
};

PBNewAction::PBNewAction( QWidget *parent ) : PBDialog( parent ) {
	...
	actionNameLE = new PBLineEdit( this );
	actionGlobLE = new PBLineEdit( this );
	actionCmdLE = new PBLineEdit( this );

	actionNameLbl->setBuddy( actionNameLE );
	actionGlobLbl->setBuddy( actionGlobLE );
	actionCmdLbl->setBuddy( actionCmdLE );

	addBtn = new PBButton( QIcon( ":/icons/list-add.png" ), this );
	addBtn->setObjectName( "okBtn" );

	cancelBtn = new PBButton( QIcon( ":/icons/delete.png" ), this );
...
QString PBNewAction::actionName() {

	return actionNameLE->text();
};

QString PBNewAction::actionCommand() {

	return actionCmdLE->text();
};

QString PBNewAction::actionFilters() {

	return actionGlobLE->text();
};

void PBNewAction::setActionName( QString str ) {

	actionNameLE->setText( str );;
};

void PBNewAction::setActionCommand( QString str ) {

	actionCmdLE->setText( str );;
};

void PBNewAction::setActionFilters( QString str ) {
