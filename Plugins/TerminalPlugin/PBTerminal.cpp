/*
	*
	* PBTerminal.cpp - PBTerminal Class
	*
*/
...
PBTerminal::PBTerminal( QString workDir, QWidget *parent ) : QMainWindow( parent ) {
...
PBTerminal::~PBTerminal() {

	for( int i = 0; i < TabWidget->count(); i++ ) {
		PBTermWidget *tw = qobject_cast<PBTermWidget*>( TabWidget->widget( i ) );
		tw->sendText( QString::fromLocal8Bit( "exit\n" ) );
	}
};

void PBTerminal::createGUI() {
...
void PBTerminal::showSettings() {

	PBTSettingsDialog *settingsDlg = new PBTSettingsDialog();
	settingsDlg->exec();

	show();
};

void PBTerminal::openFMgr() {

	QString cwd = qobject_cast<PBTermWidget*>( TabWidget->currentWidget() )->currentWorkingDirectory();
	QProcess::startDetached( "xdg-open", QStringList() << cwd );
};

void PBTerminal::closeEvent( QCloseEvent *cEvent ) {
