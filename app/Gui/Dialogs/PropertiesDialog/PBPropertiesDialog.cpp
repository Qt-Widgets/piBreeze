/*
	*
	* PBPropertiesDialog.cpp - Properties of the selected object
	*
*/
...
PBPropertiesBase::PBPropertiesBase( QStringList paths, QWidget *parent ) : QWidget( parent ) {

	pathsList << paths;

	// Name and Icon Labels
	iconLabel = new PBClickLabel( QPixmap() );
	iconLabel->setFixedSize( QSize( 48, 48 ) );
	connect( iconLabel, SIGNAL( clicked() ), this, SIGNAL( setDirIcon() ) );

	QString name;
	if ( pathsList.count() == 1 ) {
		iconLabel->setPixmap( icon( PBIconManager::instance()->iconsForFile( "", paths.at( 0 ) ) ).pixmap( QSize( 36, 36 ) ) );
...
void PBPropertiesBase::setNewIcon( QString icoStr ) {

	iconLabel->setPixmap( QIcon( icoStr ).pixmap( QSize( 36, 36 ) ) );
};

PBPropertiesWidget::PBPropertiesWidget( QStringList paths, bool *term, QWidget *parent ) : QWidget( parent ) {
...
PBPropertiesWidget::~PBPropertiesWidget() {

	*terminate = true;
	thread.waitForFinished();
};

void PBPropertiesWidget::createGUI() {
	...
	PBDriveInfo *driveInfo = new PBDriveInfo( PBDeviceManager::deviceInfoForPath( pathsList.at( 0 ) ), this );
	...
	thread = QtConcurrent::run( this, &PBPropertiesWidget::folderProperties, pathsList );
};

void PBPropertiesWidget::folderProperties( QStringList paths ) {
...
void PBPropertiesWidget::recurseProperties( QString path ) {
...
void PBPropertiesWidget::update() {
...
PBPermissionsWidget::PBPermissionsWidget( QStringList paths, QWidget *parent ) : QWidget( parent ) {
...
void PBPermissionsWidget::createGUI() {
	...
	delProtectCheck->setToolTip(
		...
		"<p><b>Note: Protection will be enabled only in PiBreeze FM. This is <u>not</u> "
	...
	PBClickLabel *r = new PBClickLabel( tr( "Read" ) );
	connect( r, SIGNAL( clicked() ), this, SLOT( setReadAll() ) );

	PBClickLabel *w = new PBClickLabel( tr( "Write" ) );
	connect( w, SIGNAL( clicked() ), this, SLOT( setWriteOwner() ) );

	PBClickLabel *x = new PBClickLabel( tr( "Exec" ) );
...
void PBPermissionsWidget::readPermissions() {
...
void PBPermissionsWidget::applyPermissions() {
...
void PBPermissionsWidget::applyTo( const char *node, QFile::Permissions perms ) {
...
void PBPermissionsWidget::applyProtection() {
...
void PBPermissionsWidget::setReadAll() {
...
void PBPermissionsWidget::setWriteOwner() {
...
void PBPermissionsWidget::setExecOwner() {
...
PBPropertiesDialog::PBPropertiesDialog( QStringList paths, PropertiesTab tab, bool *term, QWidget *parent ) : PBDialog( parent ) {

	pathsList << paths;

	setFixedSize( QSize( 530, 400 ) );

	propsB = new PBPropertiesBase( paths, this );
	propsW = new PBPropertiesWidget( paths, term, this );
	permsW = new PBPermissionsWidget( paths, this );
	...
	setDialogTitle( tr( "PiBreeze - Properties" ) );
	...
	if ( tab == PBPropertiesDialog::Properties )
		connect( propsB, SIGNAL( setDirIcon() ), this, SLOT( setDirIcon() ), Qt::UniqueConnection );
};

void PBPropertiesDialog::switchToTab( int newTab ) {
...
void PBPropertiesDialog::setDirIcon() {

	QString iconName = PBFileDialog::getFileName(
			this,
			tr( "PiBreeze - Select Icon" ),
			"/usr/share/icons/",
			QStringList(
