/*
	*
	* AboutPB.cpp - AboutPB Class for PiBreeze
	*
*/

#include "AboutPB.hpp"

AboutPB::AboutPB( QWidget *parent ) : QDialog( parent ) {

	createGui();
	setWindowProperties();
};

void AboutPB::createGui() {

	QWidget *aboutnb = createAboutPB();
	...
	PBButton *okBtn = new PBButton( QIcon::fromTheme( "dialog-ok" ), "&Close", this );
	...
	iconLbl->setPixmap( QIcon( ":/icons/pibreeze.png" ).pixmap( 48 ) );

	QLabel *textLbl = new QLabel( this );
	textLbl->setText( QString(
			"<H2>PiBreeze v3.0.0</H2>"
...
QWidget* AboutPB::createLicense() {

	QString license = QString(
		"<center><h3>PiBreeze v3.0.0</h3>"
		...
		"<p>Press Ctrl+Shift+L while running PiBreeze to see the full license.<br>"
...
void AboutPB::setWindowProperties() {
	...
	setWindowTitle( "About PiBreeze v3.0.0" );
...
License::License( QWidget *parent ) : QDialog( parent ) {
	...
	setWindowTitle( "PiBreeze v3.0.0 License" );
	setWindowIcon( QIcon( ":/icons/pibreeze.png" ) );
