/*
	*
	* PBIconManager.cpp - Icon Manager class for PiBreeze
	*
*/
...
/* Theme Database */
static QSettings mdb( PBXdg::userDir( PBXdg::XDG_CACHE_HOME ) + "PiBreeze/mimetypes.db", QSettings::NativeFormat );
static QSettings idb( PBXdg::userDir( PBXdg::XDG_CACHE_HOME ) + "PiBreeze/icons.db", QSettings::NativeFormat );
...
PBIconManager *PBIconManager::iMgr = NULL;

PBIconManager* PBIconManager::instance() {
...
QStringList PBIconManager::iconsForFile( QString mName, QString file ) {
...
bool PBIconManager::hasIcon( QString iName ) {

	return idb.allKeys().contains( iName );
};

QStringList PBIconManager::icon( QString iName ) {

	return idb.value( iName, QStringList() << ":/icons/unknown.png" ).toStringList();
};

void PBIconManager::generateThemeDatabase() {
