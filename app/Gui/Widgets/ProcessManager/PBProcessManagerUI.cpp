/*
	*
	* PBProcessManagerUI.cpp - The PBProcessManager UI class for PiBreeze
	*
*/

#include "PBProcessManagerUI.hpp"

int PBProcessManagerUI::procCount = 0;
PBProcessManagerUI* PBProcessManagerUI::procMgrUI = NULL;

PBProcessManagerUI* PBProcessManagerUI::instance() {

	if ( procMgrUI != NULL )
		return procMgrUI;

	procMgrUI = new PBProcessManagerUI();

	return procMgrUI;
};

PBProcessManagerUI::PBProcessManagerUI() : PBDialog() {

	pMgr = PBProcessManager::instance();
	connect( pMgr, SIGNAL( processAdded( PBProcess::Progress*, PBAbstractProcess* ) ), this, SLOT( addProcess( PBProcess::Progress*, PBAbstractProcess*) ) );

	createGUI();
	setWindowProperties();
};

void PBProcessManagerUI::createGUI() {
	...
	if ( pMgr->processCount() ) {
		for ( quint64 i = pMgr->processCount() - 1; i >= 0; i-- ) {
			...
			PBProcessWidget *procWidget = new PBProcessWidget( pMgr->progress( i ), pMgr->process( i ), this );
...
void PBProcessManagerUI::setWindowProperties() {

	setWindowModality( Qt::NonModal );

	setDialogTitle( "PiBreeze Process Manager" );
	setDialogIcon( QIcon( ":/icons/pibreeze.png" ) );

	setMinimumSize( QSize( 800, 500 ) );
};

void PBProcessManagerUI::show() {
...
void PBProcessManagerUI::addProcess( PBProcess::Progress* progress, PBAbstractProcess *process ) {

	procCount++;
	PBProcessWidget *procWidget = new PBProcessWidget( progress, process, this );
	baseLyt->insertWidget( 0, procWidget );
};

PBProcessManagerMini::PBProcessManagerMini( QWidget *parent ) : QToolButton( parent ) {

	/* Local instances for the ProcessManagers */
	pMgr = PBProcessManager::instance();
	procMgrUI = PBProcessManagerUI::instance();

	/* Progress fraction */
	fraction = 0;
	/* Number of active jobs */
	jobs = PBProcessManager::instance()->activeProcessCount();
...
void PBProcessManagerMini::showProcessManager() {
...
void PBProcessManagerMini::toggleProcessManager() {
...
void PBProcessManagerMini::updateTimer() {

	if ( not timer.isActive() )
		timer.start( 1000, this );
};

void PBProcessManagerMini::timerEvent( QTimerEvent *tEvent ) {
	...
	if ( tEvent->timerId() == timer.timerId() ) {
		...
		Q_FOREACH( PBProcess::Progress *prog, pMgr->activeProgresses() )
...
void PBProcessManagerMini::paintEvent( QPaintEvent *pEvent ) {
