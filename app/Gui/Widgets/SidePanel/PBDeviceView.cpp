/*
	*
	* PBDeviceView.cpp - Mounted Device View class for PiBreeze
	*
*/
...
PBDevicesIcon::PBDevicesIcon( QWidget *parent ) : QWidget( parent ) {
	...
	/* DeviceView */
	devView = new PBDeviceMenu( this );
	devView->setObjectName( "PBDeviceMenu" );
...
QPixmap PBDevicesIcon::pixmap() {

	return mPixmap;
};

void PBDevicesIcon::setPixmap( QPixmap newPixmap ) {
...
QColor PBDevicesIcon::flashColor() {

	return color;
};

void PBDevicesIcon::setFlashColor( QColor newColor ) {
...
qint64 PBDevicesIcon::alphaDelta() {

	return mAlphaDelta;
};

void PBDevicesIcon::setAlphaDelta( qint64 newAlphaDelta ) {
...
int PBDevicesIcon::flashDuration() {

	return mFlashDuration;
};

void PBDevicesIcon::setFlashDuration( int newFlashDuration ) {
...
int PBDevicesIcon::numberOfFlashes() {

	return maxFlashes;
};

void PBDevicesIcon::setNumberOfFlashes( int newNumOfFlashes ) {

	maxFlashes = newNumOfFlashes;
};

/* flashFrames property handlers - Number of frames per flash */
int PBDevicesIcon::flashFrames() {

	return flashSteps;
};

void PBDevicesIcon::setFlashFrames( int newFlashSteps ) {
...
void PBDevicesIcon::paintEvent( QPaintEvent *pEvent ) {
...
void PBDevicesIcon::mousePressEvent( QMouseEvent *mEvent ) {
...
void PBDevicesIcon::mouseMoveEvent( QMouseEvent *mEvent ) {

	mEvent->accept();
};

void PBDevicesIcon::enterEvent( QEvent *eEvent ) {
...
void PBDevicesIcon::leaveEvent( QEvent *eEvent ) {

	dndEntry = false;
	eEvent->accept();
};

void PBDevicesIcon::dragEnterEvent( QDragEnterEvent *deEvent ) {
...
void PBDevicesIcon::dragLeaveEvent( QDragLeaveEvent *dlEvent ) {

	dndEntry = false;
	dlEvent->accept();
};

void PBDevicesIcon::timerEvent( QTimerEvent *tEvent ) {
...
void PBDevicesIcon::flashLabel( QColor newColor ) {
...
void PBDevicesIcon::showDevices() {
	...
	Q_FOREACH( PBDeviceInfo info, PBDeviceManager::allDrives() ) {
		QWidgetAction *wa = new QWidgetAction( devView );
		wa->setData( info.mountPoint() );

		PBDeviceAction *actWdgt = new PBDeviceAction( info, devView );
...
void PBDevicesIcon::clickDrive( QAction *act ) {

	QWidgetAction *wAct = qobject_cast<QWidgetAction*>( act );
	PBDeviceAction *devAct = qobject_cast<PBDeviceAction*>( wAct->defaultWidget() );

	if ( devAct )
		emit driveClicked( devAct->mountPoint() );
};

PBDeviceAction::PBDeviceAction( PBDeviceInfo info, QWidget *parent ) : QWidget( parent ) {
...
QString PBDeviceAction::mountPoint() {

	return mMountPoint;
};

void PBDeviceAction::highlight( bool high ) {

	select = high;
	repaint();
}

void PBDeviceAction::paintEvent( QPaintEvent *pEvent ) {
...
void PBDeviceAction::enterEvent( QEvent *eEvent ) {
...
void PBDeviceAction::leaveEvent( QEvent *lEvent ) {

	select = false;
	repaint();
};

void PBDeviceAction::dragEnterEvent( QDragEnterEvent *deEvent ) {

	deEvent->acceptProposedAction();
};

void PBDeviceAction::dragMoveEvent( QDragMoveEvent *dmEvent ) {
...
void PBDeviceAction::dropEvent( QDropEvent *dpEvent ) {
...
PBDeviceMenu::PBDeviceMenu( QWidget *parent ) : QMenu( parent ) {

	entered = false;
	connect( this, SIGNAL( hovered( QAction* ) ), this, SLOT( highlightAction( QAction* ) ) );
};

void PBDeviceMenu::clear() {

	QMenu::clear();
	actionList.clear();
};

void PBDeviceMenu::addAction( QWidgetAction *act ) {

	actionList << act;
	QMenu::addAction( act );
};

void PBDeviceMenu::highlightAction( QAction *act ) {

	/* Remove highlighting from all QWidgetActions */
	Q_FOREACH( QWidgetAction *wa, actionList ) {
		PBDeviceAction *devAct = qobject_cast<PBDeviceAction*>( wa->defaultWidget() );
			if ( devAct )
...
void PBDeviceMenu::enterEvent( QEvent *eEvent ) {

	entered = true;
	eEvent->accept();
};

void PBDeviceMenu::close() {
