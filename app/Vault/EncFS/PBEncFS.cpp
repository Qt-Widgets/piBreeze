/*
	*
	* PBEncFS.cpp - PiBreeze Folder Encryption Class
	*
*/

#include "PBEncFS.hpp"

PBEncFS::PBEncFS( QString source, QString target, QWidget *parent ) : QObject( parent ) {
...
bool PBEncFS::mountDir( QString password ) {

	if ( not exists( mTarget ) ) {
		if ( mkpath( mTarget ) ) {
			QMessageBox::critical(
				mParent,
				"PiBreeze - Mount Falied",
				QString(
					"PiBreeze failed to create the target directory <tt>%1</tt>. "
	...
	if ( not text.count() ) {
		...
		if ( proc.exitCode() ) {
			QMessageBox::critical(
				mParent,
				"PiBreeze EncFS - Mount Failed",
				"PiBreeze falied to mount the given encfs volume. Please check if you have provided the correct password."
			);
...
bool PBEncFS::createEncFS( QString passwd ) {
	...
	if ( proc.exitCode() ) {
		...
		QMessageBox::information(
			mParent,
			"PiBreeze - EncFS Creation Falied",
	...
	QMessageBox::information(
		mParent,
		"PiBreeze - EncFS Creation Success",
	...
	if ( exists( mTgtTmp ) ) {
		...
		if ( ret ) {
			QMessageBox::information(
				mParent,
				"PiBreeze - Data Restore Failed",
				QString( "PiBreeze has encountered error in restoring the data that existed in your target directory from the temporary location. "
...
bool PBEncFS::changePass( QString oPassword, QString nPassword ) {
...
PBCreateEncFS::PBCreateEncFS( QWidget *parent ) : QDialog( parent ) {
...
QString PBCreateEncFS::directoryName() {
...
void PBCreateEncFS::updateButton() {
