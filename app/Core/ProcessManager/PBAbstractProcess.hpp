/*
	*
	* PBAbstractProcess.hpp - PBAbstractProcess.cpp header
	*
	* PBProgress is a struct that contains all the necessary details for showing a progress dialog.
	*
	...
	*   - progressText            Text to be shown by the PBProgressBar (optional)
	*   - type                    Type of process (See PBProcess::Type)
	*   - state                   Type of process (See PBProcess::State)
	*
	* Note: Path for all the sources that need to be processed must be relative to PBProgress::sourceDir
	* Example:
...
namespace PBProcess {
	...
	typedef struct PBProgress_t {
		...
		PBProcess::State state;
	} Progress;
};

class PBAbstractProcess : public QThread {
	Q_OBJECT
