...
PBCustomPeek::PBCustomPeek( QString pth, QWidget *parent ) : QWidget( parent ) {
...
void PBCustomPeek::setWindowProperties() {
...
void PBCustomPeek::keyPressEvent( QKeyEvent *keyEvent ) {
...
void PBCustomPeek::changeEvent( QEvent *event ) {
...
void PBCustomPeek::paintEvent( QPaintEvent *pEvent ) {
...
void PBCustomPeek::getFileProps() {

	iconLbl->setPixmap( icon( PBIconManager::instance()->iconsForFile( "", path ) ).pixmap( QSize( 64, 64 ) ) );
	QFileInfo info( path );
...
void PBCustomPeek::openInExternal() {
