/*
	*
	* PBSalsa20.cpp - PiBreeze File Encryption Class
	*
*/

#include "PBSalsa20.hpp"
#include <iostream>

PBSalsa20::PBSalsa20( QString path, QWidget *parent ) : QObject() {
...
void PBSalsa20::makeIVFromString( QByteArray text, uint8_t *iv ) {
...
void PBSalsa20::makeKeyFromString( QByteArray text, uint8_t *key ) {
...
void PBSalsa20::encrypt( QByteArray password ) {
	...
	// if ( exists( mFilePath + ".s20" ) ) {
		...
		// if ( rename( on.toLocal8Bit().data(), ( on  + ".orig" ).toLocal8Bit().data() ) != 0 ) {
			// QMessageBox::information(
				// mParent,
				// "PiBreeze - File Encryption Failed",
...
void PBSalsa20::decrypt( QByteArray password ) {
...
QByteArray PBSalsa20::encryptData( QByteArray data, QByteArray password ) {
...
QByteArray PBSalsa20::decryptData( QByteArray data, QByteArray password ) {
