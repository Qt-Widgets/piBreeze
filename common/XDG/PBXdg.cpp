/*
	*
	* PBXdg.cpp - PBXdgimplementation for NewBreeze
	*
*/
...
QString PBXdg::xdgDefaultApp( QString mimeType ) {
	...
	*/
	...
	QStringList files;
	files << PBXdg::home() + QString( "/.config/mimeapps.list" );
	files << PBXdg::home() + QString( "/.local/share/applications/mimeapps.list" );
	files << PBXdg::home() + QString( "/.local/share/applications/defaults.list" );
	files << PBXdg::home() + QString( "/.local/share/applications/mimeinfo.cache" );
...
QStringList PBXdg::systemDirs( PBXdg::XdgSystemDirs pathEnum ) {

	switch( pathEnum ) {
		case PBXdg::XDG_DATA_DIRS : {
...
QString PBXdg::trashLocation( QString path ) {

	/* Ideally, if path is inside home */
	if ( path.startsWith( home() ) ) {
		return PBXdg::homeTrashLocation();
	}

	else {
		PBDeviceInfo trashDevInfo = PBDeviceManager::deviceInfoForPath( path );
...
QString PBXdg::homeTrashLocation() {
