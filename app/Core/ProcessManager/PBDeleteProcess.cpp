/*
	*
	* PBDeleteProcess.cpp - PiBreeze Process Class for file I/O
	*
*/

#include "PBDeleteProcess.hpp"

PBDeleteProcess::PBDeleteProcess( QStringList sources, PBProcess::Progress *progress ) {
...
QStringList PBDeleteProcess::errors() {

	return errorNodes;
};

void PBDeleteProcess::cancel() {

	mProgress->state = PBProcess::Canceled;
	mCanceled = true;
};

void PBDeleteProcess::pause() {

	mProgress->state = PBProcess::Paused;
	mPaused = true;
};

void PBDeleteProcess::resume() {

	mProgress->state = PBProcess::Started;
	mPaused = false;
};

bool PBDeleteProcess::canUndo() {

	return false;
};

void PBDeleteProcess::undo() {

};

void PBDeleteProcess::deleteNode( QString path ) {
...
void PBDeleteProcess::trashNode( QString node ) {
	...
	QString trashLoc = PBXdg::trashLocation( mProgress->sourceDir );
...
void PBDeleteProcess::restoreNode( QString path ) {

	Q_UNUSED( path );
};

void PBDeleteProcess::run() {

	/* Undo for PBProcess::Copy */
	if ( mUndo ) {
	...
	Q_FOREACH( QString path, sourceList ) {
		if ( mProgress->type == PBProcess::Delete )
			deleteNode( path );
