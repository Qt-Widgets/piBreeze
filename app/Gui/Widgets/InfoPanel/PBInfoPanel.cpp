/*
	*
	* PBInfoPanel.cpp - InfoPanel class for PiBreeze
	*
*/
...
PBInfoPanel::PBInfoPanel( QWidget *parent ) :QWidget( parent ) {
...
void PBInfoPanel::updatePanel( QString rootPath, QModelIndexList selection ) {
	...
	if ( rootPath.startsWith( "PB://SuperStart" ) ) {
	...
	else if ( rootPath.startsWith( "PB://Applications" ) ) {
		if ( selection.isEmpty() ) {
			setIcon( QIcon( ":/icons/applications.png" ) );
			nameLbl->setText( "<b>Applications</b>");
			pathLbl->setText( "Run an application without leaving PB3" );
		}
	...
	else if ( rootPath.startsWith( "PB://Catalogs" ) ) {
		if ( selection.isEmpty() ) {
			setIcon( QIcon( ":/icons/superstart.png" ) );
		...
		if ( selection.isEmpty() ) {
			setIcon( icon( PBIconManager::instance()->iconsForFile( "inode/directory", rootPath ) ) );
			if ( rootPath.count( "/" ) > 1 )
			...
			setIcon( icon( PBIconManager::instance()->iconsForFile( mType.name(), selection.at( 0 ).data( Qt::UserRole + 7 ).toString() ) ) );
			nameLbl->setText( "<b>" + baseName( rootPath ) + "</b>" );
...
void PBInfoPanel::setIcon( QIcon icon ) {

	iconLbl->setPixmap( icon.pixmap( QSize( 128, 128 ) ) );
};

void PBInfoPanel::setIcon( QPixmap icon ) {

	iconLbl->setPixmap( icon );
};

void PBInfoPanel::paintEvent( QPaintEvent *pEvent ) {
