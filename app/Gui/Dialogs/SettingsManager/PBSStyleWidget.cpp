/*
	*
	* PBSStyleWidget.cpp - Style Settings Widget for PiBreeze
	*
*/

#include "PBSStyleWidget.hpp"

PBSStyleWidget::PBSStyleWidget( QWidget * parent ) : QWidget( parent ) {
	...
	preview = new PBSSPreviewWidget( this );

	segBtn = new PBSegmentButton( this );
	segBtn->setCount( 1 );
...
void PBSStyleWidget::stylePreview( const QString style ) {

	preview->showStyle( style );
};

PBSSPreviewWidget::PBSSPreviewWidget( QWidget *parent ) : QWidget( parent ) {
...
void PBSSPreviewWidget::showStyle( const QString style ) {
...
void PBSStyleWidget::applyStyle() {
