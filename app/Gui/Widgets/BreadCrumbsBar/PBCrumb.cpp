/*
	*
	* PBCrumb.cpp - Alternate PushButton/ToolButton for PiBreeze
	*
*/

#include "PBCrumb.hpp"

PBCrumb::PBCrumb( QString text, bool currentPath ) : QWidget() {
...
QSize PBCrumb::sizeHint() const {

	return QSize( mWidth, 24 );
};

QString PBCrumb::path() {

	return mPath;
};

void PBCrumb::showMenu() {

	m_MenuButtonPressed = true;
	repaint();

	QMenu *menu = new QMenu( this );
	if ( mPath.startsWith( "PB://SuperStart" ) ) {
...
void PBCrumb::onMenuItemClicked() {

	emit loadPath( qobject_cast<QAction *>( sender() )->data().toString() );
};

void PBCrumb::mousePressEvent( QMouseEvent *mEvent ) {
...
void PBCrumb::mouseReleaseEvent( QMouseEvent *mEvent ) {
...
void PBCrumb::enterEvent( QEvent *eEvent ) {

	repaint();
	eEvent->accept();
};

void PBCrumb::leaveEvent( QEvent *lEvent ) {

	repaint();
	lEvent->accept();
};

void PBCrumb::paintEvent( QPaintEvent *pEvent ) {
