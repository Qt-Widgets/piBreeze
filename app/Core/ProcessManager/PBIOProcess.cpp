/*
	*
	* PBIOProcess.cpp - PiBreeze Process Class for file I/O
	*
	* This is the heart of copy/move operations.
	*
	* PBProcess::Progress is defined in the header PBAbstractProcess.hpp
...
PBIOProcess::PBIOProcess( QStringList sources, PBProcess::Progress *progress ) {
	...
	/* Initialize the state to PBProcess::NotStarted */
	mProgress->state = PBProcess::NotStarted;
};

QStringList PBIOProcess::errors() {

	return errorNodes;
};

void PBIOProcess::cancel() {

	mProgress->state = PBProcess::Canceled;
	mCanceled = true;
};

void PBIOProcess::pause() {

	mProgress->state = PBProcess::Paused;
	mPaused = true;
};

void PBIOProcess::resume() {

	mProgress->state = PBProcess::Started;
	mPaused = false;
};

bool PBIOProcess::canUndo() {

	return false;
};

void PBIOProcess::undo() {

};

bool PBIOProcess::preIO() {

	mProgress->state = PBProcess::Starting;

	if ( not isWritable( mProgress->targetDir ) ) {
		emit noWriteAccess();
		mProgress->state = PBProcess::Completed;

		return false;
	}

	/* If we are moving file and in the same partition, we can skip the processing */
	if ( mProgress->type == PBProcess::Move ) {
	...
	PBDeviceInfo tgtInfo = PBDeviceManager::deviceInfoForPath( mProgress->targetDir );
...
void PBIOProcess::processDirectory( QString path ) {
...
void PBIOProcess::copyFile( QString srcFile ) {
	...
	/* PBProcess::Progress::fileBytes */
	mProgress->fileBytes = iStat.st_size;

	/* PBProcess::Progress::fileBytesCopied */
	mProgress->fileBytesCopied = 0;
